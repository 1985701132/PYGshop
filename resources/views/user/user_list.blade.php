...
<div class="page-content clearfix">
    ...
    <div class="search_style">
      ...
      <form action="">
       <li><label class="l_f">会员名称</label><input name="keyword" value="{{ $req->keyword }}" type="text"  class="text_add" placeholder="输入会员名称、电话、邮箱"  style=" width:400px"/></li>
       <li><label class="l_f">添加时间</label><input name="time" value="{{$req->time}}" class="inline laydate-icon" id="start" style=" margin-left:10px;"></li>
  ...
  @foreach($users as $v)
		<tr>
          ...
          <td>
            @if($v->gender=='man')
            男
            @elseif($v->gender=='woman')
            女
            @elseif($v->gender=='secret')
            保密
            @endif
          </td>
          <td>{{ $v->mobile }}</td>
          ...
          <td>{{ $v->created_at }}</td>
          <td>普通用户</td>
          <td class="td-status"><span @if($v->is_use=='yes') class="label label-success radius" @elseif($v->is_use=='no') class="label label-defaunt radius" @endif >@if($v->is_use=='yes') 已启用 @elseif($v->is_use=='no') 已停用 @endif</span></td>
          <td class="td-manage">
          <a  name="is_use" value="no" href="javascript:;" @if($v->is_use=='yes') title="停用" class="btn btn-xs btn-success" onClick="member_stop(this,{{ $v->id }})" @elseif($v->is_use=='no') title="启用" class="btn btn-xs" onClick="member_start(this,{{ $v->id }})" @endif ><i class="icon-ok bigger-120"></i></a> 
	...
	{{ $users->appends($req->except('page'))->links() }}
...
<form action="{{ route('user.insert') }}" method="post" id="form-admin-add">
	{{ csrf_field() }}
    <ul class=" page-content">
     ...
     <label><input name="acc" type="radio" value="man"  class="ace"><span class="lbl">男</span></label>&nbsp;&nbsp;&nbsp;
     <label><input name="acc" type="radio" value="woman" class="ace"><span class="lbl">女</span></label>&nbsp;&nbsp;&nbsp;
     <label><input name="acc" type="radio" value="secret"  class="ace"><span class="lbl">保密</span></label>
     </span>
...
function member_stop(obj,id){
    ...
    $.ajax({
      ...
      success:function(data){
        	$(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none" value="is_use" class="btn btn-xs " onClick="member_start(this,id)" href="javascript:;" title="启用"><i class="icon-ok bigger-120"></i></a>');
...
function member_start(obj,id){
    ...
    $.ajax({
      ...
      data:{id:id},
      success:function(data){
        $(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none" value="is_use" class="btn btn-xs btn-success" onClick="member_stop(this,id)" href="javascript:;" title="停用"><i class="icon-ok bigger-120"></i></a>');
