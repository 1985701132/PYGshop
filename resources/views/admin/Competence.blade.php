  ...
  <div class="left_Competence_add">
   <div class="title_name">添加权限</div>
    ...
    <div class="form-group"><label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 用户选择 </label>
       <div class="col-sm-9">
		 @foreach($admins as $v) 
	   <label class="middle"><input name="username[]" value="{{ $v->id }}" class="ace" type="checkbox" id="id-disable-check"><span class="lbl"> {{ $v->username }}</span></label>
		@endforeach
	</div>   
   </div>
   <!--按钮操作-->
   ...
   <div class="Assign_style">
	...
	@foreach($data as $v)
	  ...
	  <dt><label class="middle"><input name="user-Character-0" class="ace" type="checkbox" id="id-disable-check"><span class="lbl">{{ $v->pri_name }}</span></label></dt>
		<dd>
			@foreach($v->child as $v2)
        <dl class="cl permission-list2">
		<dt><label class="middle"><input type="checkbox" value="" class="ace"  name="user-Character-0-1" id="user-Character-0-1"> <span class="lbl">{{ $v2->pri_name }}</span></label></dt>
		  <dd>
			  @foreach($v2->child as $v3)
		  <label class="middle"><input type="checkbox" value="{{ $v3->id }}" class="ace" name="pri_name[]" id="user-Character-0-0-0"><span class="lbl">{{ $v3->pri_name }}</span></label>
		   @endforeach
		 </dd>
		</dl>
		@endforeach
